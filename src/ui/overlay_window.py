...
class OverlayWindow(tk.Toplevel):
    ...
    def __init__(
        ...
        # ── 拖拽状态 ──────────────────────────────────────────────────────
        self._drag_data = {}  # 记录拖拽起始信息（模式、起始坐标、初始窗口几何）
    ...
    def _resize_do(self, event: tk.Event) -> None:
        ...
        d = self._drag_data
        dx = event.x_root - d["start_x"]  # 鼠标相对起始位置的水平位移
        dy = event.y_root - d["start_y"]  # 鼠标相对起始位置的垂直位移
        edge = d["edge"]

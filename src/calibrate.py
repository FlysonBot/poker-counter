...
def _calibrate(frame: GrayImage, window_rect: Rect) -> float:
    ...
    row_sums = np.sum(block_rows == 255, axis=1)
    row_density = row_sums / bw
    dense_rows = np.where(row_density >= _ROW_DENSITY_MIN)[0]
    ...
    logger.info(f"scale 自动校准完成：牌高={card_height}px，参考高度={_CARD_REF_HEIGHT}px，scale={scale:.3f}")
    return scale
